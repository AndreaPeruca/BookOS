        ...
        if len(snapshots) < 2:
            ...
            if not PLOTLY:
                ...
                fig_val.add_trace(go.Scatter(
                    ...
                    name="",
                    showlegend=False,
                    line=dict(color="#B5362C", width=3),
                    marker=dict(
                        size=11, color="#B5362C",
                        line=dict(width=2.5, color="white"),
                    ),
                    fill="tozeroy",
                    fillcolor="rgba(181,54,44,0.07)",
                    hovertemplate="<b>%{x}</b><br>Valore magazzino: %{customdata}<extra></extra>",
                    customdata=[fmt_euro(v) for v in df_agg["Valore_Mag"]],
                ))
                # Etichette dati ai nodi — stile editoriale
                ...
                _econ(
                    ...
                    subtitle="Valore a costo dei libri in giacenza · €",
                    src="Elaborazione su dati del gestionale",
                )
                fig_val.update_yaxes(
                    tickprefix="€ ", tickformat=",.0f",
                    range=[0, df_agg["Valore_Mag"].max() * 1.30],
                )
                ...
                if df_st_ed is not None and len(top_editori) > 0:
                    ...
                    for i, editore in enumerate(top_editori):
                        ...
                        fig_st.add_trace(go.Scatter(
                            ...
                            hovertemplate=(
                                f"<b>{editore}</b><br>"
                                "Sell-through: %{y:.1f}%/mese<extra></extra>"
                            ),
                            connectgaps=True,
                        ))
                    _econ(
                ...
                if tit_fermi.empty:
                    ...
                    tit_d["_Label"] = tit_d["Titolo"].apply(
                        lambda t: t[:42] + "…" if len(t) > 42 else t
                    )
                    ...
                    alpha_vals    = 0.40 + 0.60 * tit_d["Pct_Fermo"]
                    ...
                    fig_fermi = go.Figure(go.Bar(
                        ...
                        textfont=dict(size=10, color="#16130F"),
                        hovertemplate=(
                            "<b>%{y}</b><br>"
                            "Valore imm. medio: %{x:,.0f} €<br>"
                    ...
                    _econ(
                        ...
                        subtitle=(
                            "Valore medio immobilizzato (€ a costo) · "
                            "colore più intenso = fermo in più snapshot"
                        ),
                        src="Elaborazione su dati del gestionale",
                    )
                    fig_fermi.update_xaxes(tickprefix="€ ", tickformat=",.0f")
                    fig_fermi.update_layout(
                        height=max(320, 44 * len(tit_d) + 130),
                        margin=dict(l=8, r=130, t=84, b=76),
                    )
                ...
                if df_delta_ed is not None and len(df_delta_ed) > 0:
                    ...
                    fig_delta = go.Figure(go.Bar(
                        ...
                        hovertemplate=(
                            "<b>%{y}</b><br>"
                            f"Sell-through {periodo_labels[0]}: %{{customdata[0]:.1f}}%<br>"
                            f"Sell-through {periodo_labels[-1]}: %{{customdata[1]:.1f}}%<br>"
                            "Variazione: %{x:+.1f}pp<extra></extra>"
                        ),
                        customdata=list(zip(
                    ...
                    fig_delta.update_layout(
                        height=max(320, 44 * len(df_delta_ed) + 130),
                        margin=dict(l=8, r=80, t=84, b=76),
                    )
    ...
    if df_sim_ref is None:
        ...
        if df_pool.empty:
            ...
            if PLOTLY:
                ...
                nomi_sc  = ["Prudente", "Base", "Ottimista"]
                ...
                fig_sim = go.Figure(go.Bar(
                    ...
                    marker_line_width=0,
                    text=[f"  {c} cop. · {cov}" for c, cov in zip(copie_sc, cov_sc)],
                    textposition="outside",
                    textfont=dict(size=11, color="#16130F"),
                    hovertemplate=(
                        "<b>%{y}</b><br>"
                        "Copie: %{x}<br>"
                        "Vel. vendita: %{customdata[0]:.1f} cop./mese<br>"
                        "Copertura: %{customdata[1]}<br>"
                        "Investimento: %{customdata[2]}<extra></extra>"
                    ),
                ...
                _econ(
                    ...
                    subtitle=(
                        f"Tre scenari · copertura obiettivo {sim_target_mesi} mesi"
                        + (f" · editore: {sim_editore}" if sim_editore.strip() else "")
                    ),
                    src="Elaborazione su dati del gestionale",
                )
                fig_sim.update_xaxes(
                    range=[0, max(copie_sc) * 1.55],
                    title=None,
                    dtick=1 if max(copie_sc) <= 20 else None,
                )
                fig_sim.update_layout(height=320, margin=dict(l=8, r=140, t=84, b=76))
