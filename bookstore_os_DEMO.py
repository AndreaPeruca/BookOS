...
# ---------------------------------------------------------------------------
# DEMO MODE: Precarica dati di test
# ---------------------------------------------------------------------------
if st.session_state.get("df_mag") is None:
    try:
        import os
        demo_path = os.path.join(os.path.dirname(__file__), "storico_apr2024.csv")
        if os.path.exists(demo_path):
            df_demo = pd.read_csv(demo_path)
            df_demo = normalize_columns(df_demo)
            if validate_schema(df_demo, SCHEMA_MAGAZZINO, "Demo Data"):
                st.session_state["df_mag"] = df_demo
                st.session_state["df_mag_name"] = "storico_apr2024.csv [DEMO]"
    except Exception as e:
        import logging
        logging.error(f"Demo load error: {e}")
...
with st.sidebar:
    ...
    # DEMO: Precarica file di test se non caricato
    if mag_file_sb is None and st.session_state.get("df_mag") is None:
        try:
            import pathlib
            demo_file = pathlib.Path(__file__).parent / "storico_apr2024.csv"
            if demo_file.exists():
                ...
                if validate_schema(df_demo, SCHEMA_MAGAZZINO, "Demo Data"):
                    st.session_state["df_mag"] = df_demo
                    st.session_state["df_mag_name"] = "storico_apr2024.csv [DEMO]"
                    st.toast("Dati DEMO caricati automaticamente", icon="OK")
        except:
            pass
